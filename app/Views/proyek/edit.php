...
<body>
    <div class="container">
        ...
        <h2><i class="fa-solid fa-pen-to-square"></i> Edit Proyek</h2>
        <form method="post" action="/proyek/update/<?= $proyek['id'] ?>" enctype="multipart/form-data">
            <div class="form-group">
                <label for="judul"><i class="fa-solid fa-book"></i> Judul</label>
                <input type="text" name="judul" id="judul" value="<?= htmlspecialchars($proyek['judul']) ?>" required>
            </div>
            <div class="form-group">
                <label for="pembimbing"><i class="fa-solid fa-user-tie"></i> Pembimbing</label>
                <input type="text" name="pembimbing" id="pembimbing" value="<?= htmlspecialchars($proyek['pembimbing']) ?>" required>
            </div>
            <div class="form-group">
                <label for="deadline"><i class="fa-solid fa-calendar-day"></i> Deadline</label>
                <input type="date" name="deadline" id="deadline" value="<?= htmlspecialchars($proyek['deadline']) ?>" required>
            </div>
            <div class="form-group">
                <label for="progress"><i class="fa-solid fa-bolt"></i> Progress (%)</label>
                <input type="number" name="progress" id="progress" min="0" max="100" value="<?= htmlspecialchars($proyek['progress']) ?>" required>
            </div>
            <div class="form-group">
                <label for="catatan"><i class="fa-solid fa-note-sticky"></i> Catatan</label>
                <textarea name="catatan" id="catatan"><?= htmlspecialchars($proyek['catatan']) ?></textarea>
            </div>
            <div class="form-group">
